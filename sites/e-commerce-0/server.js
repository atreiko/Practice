...
let serviceAccount = require("./ecom-website-29cbd-firebase-adminsdk-u46zz-143a279ff8.json");
...
const bucketName = 'e-commerce-0e'
    ...
    let { name, 
        ...
        if(!draft) {
            if(!name.length) {
                ...
                return res.json({ 'alert':'short description must between 10 to 100 letters long' })
            } else if(!des.length) {
                return res.json({ 'alert':'enter detail description about the product' })
            } else if(!images.length) { //* image link array
                return res.json({ 'alert':'upload at least one product' })
